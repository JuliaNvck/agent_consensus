...
%  Conference paper draft -- compile with pdflatex + bibtex
...
% Figure base path: paper/latex/ -> project root is ../../
\graphicspath{{../../}}
...
% ============================================================
\begin{abstract}
Multi-agent LLM ensembles often aggregate independent completions using
self-consistency, a majority vote over extracted answers. This paper studies
two failure modes of that procedure and argues that they require different
defenses rather than a single universally optimal consensus rule. Invalid-format
...
Across experiments on LLaMA 3.1 8B and Qwen2.5 7B using 100 questions from
GSM8K and StrategyQA, strict answer extraction corrects a 16.5 percentage point
accuracy collapse in LLaMA majority-vote results at high fault fractions, and
TopKMass filtering plus strict majority vote has the highest observed average
accuracy under invalid-format faults. TopKMass is the strongest of three tested
...
provides little guidance about how to handle malformed outputs, confidence
signals, or coordinated wrong answers.

We study these issues through a failure-mode-specific lens. The core observation
is that self-consistency can fail for qualitatively different reasons, and
defenses that help in one regime can be neutral or harmful in another.
...
attack, and proposes complementary mechanisms to address each failure mode. The
strict extraction fix (abstaining when no parseable answer is found) is itself a
methodology improvement applicable to any self-consistency implementation.
...
text semantics). The two approaches address different failure modes; DecentLLMs
does not expose per-token logprobs and therefore cannot perform
...
fault fraction regime ($\beta=0.45$ with $N=7$ corresponds to $f=3$, which is
super-threshold and causes consistent liveness fallback).
...
is the background for Module~1's signal design. TopKMass is a sliding-window
stability measure (the mean sum of top-5 token probabilities over a $W=64$
window) rather than an instantaneous per-token confidence. This design captures
whether a model maintains high-confidence generation across a sequence, which is
more predictive of output reliability than any single token's confidence.
...
\begin{lstlisting}[language=Python, caption={Data contracts between pipeline modules.}]
@dataclass
...
\begin{equation}
\mathrm{TopKMass}(t) = \frac{1}{\min(t+1,\,W)}
  \sum_{i=\max(0,\,t-W+1)}^{t}
  \sum_{k=1}^{5} \exp\!\bigl(\mathrm{logprob}_{i,k}\bigr)
\label{eq:topkmass}
\end{equation}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Post-warmup:} skip the first $W$ positions, where the window
        is only partially filled and scores are systematically lower than the
        stable-state value. The stable portion starts at position $W=64$.
...
\texttt{max\_tokens}$=128$), a large fraction of the trajectory is in the warmup
region where scores are systematically lower than the stable-state value.
Without correction, a single global threshold $\tau$ would penalize short
outputs relative to long ones. The stable-region mean is computed on the
trajectory slice at positions $[W{:}]$, falling back to the full trajectory when
output length is at most $W$. This is applied identically in both filtering and
$\tau$ calibration, ensuring the same metric is used throughout.
...
Clean agent
  & $[0.96,\,1.00]$ (typical range)
  & Passes unless rare outlier \\
\bottomrule
\end{tabular}
\end{table}

F2 Byzantine logprobs are spoofed to TopKMass$=1.00$ (not 0.9999) because
high-precision models (notably Qwen2.5) produce clean-agent scores up to
${\approx}0.9951$; the exact value 1.00 is required to avoid inadvertent
filtering of Byzantine agents.
...
empty \texttt{output\_text} and \texttt{token\_logprobs}. F2 agents receive a
pre-specified wrong answer string and logprobs spoofed to TopKMass$=1.00$. F3
agents receive syntactically plausible off-task text and token logprobs set
uniformly to $-10.0$, producing a TopKMass score of approximately
$2.3\times10^{-4}$ and causing reliable filter rejection. The \texttt{mix}
...
\{F1, F2, F3, mix\} on 100 questions (50 GSM8K multi-step arithmetic,
...
StrategyQA). $\tau$ is auto-calibrated per run.
...
percentage point LLaMA accuracy collapse at $\beta=0.45$, which this methodology
corrects.
...
\begin{table}[h]
\centering
\caption{LLaMA 3.1 8B accuracy by condition and fault fraction (averaged over
  $N \in \{5,7\}$ and all fault types).}
\label{tab:exp1-llama}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Condition} & \textbf{N=1 (ref)} & $\bm{\beta}=0$\%
  & $\bm{\beta}=15$\% & $\bm{\beta}=30$\%
  & $\bm{\beta}=45$\% \\
\midrule
\texttt{baseline} (self-consistency) & 0.713 & 0.756 & 0.769 & 0.736 & 0.708 \\
\texttt{soft\_weighting}             & 0.713 & 0.713 & 0.725 & 0.728 & 0.688 \\
...
\begin{table}[h]
\centering
\caption{Qwen2.5 7B accuracy by condition and fault fraction (averaged over
  $N \in \{5,7\}$ and all fault types).}
\label{tab:exp1-qwen}
\small
\begin{tabular}{lccccc}
...
\textbf{Condition} & \textbf{N=1 (ref)} & $\bm{\beta}=0$\%
  ...
  & $\bm{\beta}=45$\% \\
\midrule
\texttt{baseline} (self-consistency) & 0.675 & 0.669 & 0.669 & 0.673 & 0.648 \\
\texttt{soft\_weighting}             & 0.675 & 0.650 & 0.650 & 0.644 & 0.663 \\
\texttt{hard\_only}                  & 0.675 & 0.669 & 0.669 & 0.675 & \textbf{0.656} \\
...
strict extraction it is 0.708, a 16.5pp improvement. The gap arose because F2
and F3 agents with unscorable outputs contributed their full text as vote keys,
which could win plurality when parseable-answer agents did not form a single
...
\paragraph{Finding 2: \texttt{hard\_only} has the highest observed average
accuracy for invalid-format faults.}
...
faults.}
At $\beta=0.45$, \texttt{full\_system} is 5.4pp below \texttt{hard\_only} for
...
fallback on 13.75\% of Qwen questions (vs.\ 5.0\% for LLaMA). Qwen's
stable-region TopKMass scores cluster lower than LLaMA's, so the 5th-percentile
$\tau$ cuts more clean Qwen agents. This is a calibration artifact; accuracy
recovers via liveness fallback, but the elevated fallback rate reduces the
filter's net contribution. Adaptive per-model $\tau$ calibration with larger dev
slices could reduce this fallback rate.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/accuracy_vs_beta_no_n1}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fault_type_breakdown}
...
\toprule
\textbf{Signal}
  & \textbf{LLaMA AUC} & \textbf{LLaMA AP}
  & \textbf{Qwen AUC}  & \textbf{Qwen AP} \\
\midrule
\textbf{TopKMass}        & \textbf{0.612} & \textbf{0.810} & \textbf{0.635} & \textbf{0.776} \\
...
Both correct and incorrect agents cluster in a narrow high-confidence range
($[0.96,\,1.00]$ for LLaMA, $[0.94,\,1.00]$ for Qwen) with heavy overlap. The
correct-agent TopKMass median is higher than the incorrect-agent median (0.988
vs.\ 0.981 for LLaMA, a gap of 0.007; see Figure~\ref{fig:topkmass-dist}, which
shows the distribution for clean LLaMA agents). TopKMass's primary value as a
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/exp2_llama/topkmass_distribution}
\caption{Histogram of TopKMass score distributions for correct vs.\ incorrect
  ...
  lines mark class medians (correct: 0.988, incorrect: 0.981). Fault-agent
  scores (F1: 0.0, F3: ${\approx}2.3\times10^{-4}$) are far off-scale to the
  left and not shown.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{results/exp2_llama/experiment_2_signals}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Coordinated:} Both agents produce the same wrong answer derived
        ...
        in embedding space.
  \item \textbf{Maximally adversarial:} Identical to coordinated in answer
        ...
        equivalent to coordinated for Module~1 admission and is interpreted as a
...
\multirow{2}{*}{Coordinated}
  & LLaMA & 0.353 & 0.113 & \textbf{$+$0.240} \\
  & Qwen  & 0.366 & 0.076 & \textbf{$+$0.290} \\
\midrule
\multirow{2}{*}{Max adversarial}
  & LLaMA & 0.353 & 0.113 & \textbf{$+$0.240} \\
  & Qwen  & 0.366 & 0.076 & \textbf{$+$0.290} \\
\bottomrule
\end{tabular}
...
agents unify on a single wrong answer, reducing majority vote accuracy to 62.5\%.
...
median nearest-centroid is 11.25\% (vs.\ 2.5\% for LLaMA): Qwen's lower
TopKMass distribution causes Module~1 to over-filter clean agents, reverting to
the full 7-agent pool on roughly 1 in 9 questions. The centroid shift deltas
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/adversarial_coordination}
\caption{Experiment~3: accuracy by coordination degree (left) and centroid shift
  ($\delta$) by coordination degree and model (right).}
...
The multi-provider pool does not exceed the strongest single provider:
...
agents, does not provide an observed accuracy advantage in this setup.

\subsection{Experiment B: Biased Provider Injection}
\label{sec:exp-b}

Experiment~B tests whether geometric median can absorb a systematically biased
...
modes of self-consistency require distinct mechanisms to address, and no single
pipeline condition is optimal for both.
...
F1 crash / F3 drifter (invalid-format)
  ...
  & Module~1 drops low-confidence agents; liveness fallback matters
    when overloaded \\[6pt]
F2 Byzantine (spoofed logprob, scattered)
  & \texttt{hard\_only}
...
ineffective against F2 Byzantine agents (TopKMass$=1.00$ by design), because
...
centroid shift results ($+0.240$ to $+0.290$ embedding units, positive across
...
The first is bidirectional NLI entailment verification. It was designed to check
...
The second is distance-weighted answer voting. This is the \texttt{full\_pipeline}
condition in the reported Experiment~3 results. It underperforms \texttt{stage1\_only}
...
this setting, no agent has an incentive to deceive; F2 Byzantine injection is a
synthetic stress test, not a real-world threat. The BFT framing becomes more
...
F2 Byzantine agents use a fixed wrong-answer template and perfectly spoofed
logprobs. Real-world failure modes may have different logprob signatures or
...
The primary Experiment~1 result is a methodology finding: strict answer
extraction corrects a 16.5 percentage point accuracy collapse in LLaMA
majority-vote results at high fault fractions, and TopKMass filtering plus
strict majority vote has the highest observed average accuracy for invalid-format
faults. Experiment~2 validates TopKMass as the strongest of the tested
...
cluster; majority vote remains simpler and at least as effective under several
...
\section{Figure: Accuracy vs. Fault Fraction (with N=1 Reference)}
\label{app:figures}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/accuracy_vs_beta}
...
\section{Figure: Qwen Signal Quality}
\label{app:qwen-signals}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{results/exp2_qwen/experiment_2_signals}
\caption{Experiment~2 (Qwen2.5 7B): signal quality panels, parallel to
  ...
  vs.\ 0.032 for LLaMA). Logprob variance remains below chance for Qwen
  (AUC$=0.538$, though above the LLaMA value of 0.448). The narrower TopKMass
  score range for Qwen ($[0.94,\,1.00]$) vs.\ LLaMA ($[0.79,\,1.00]$) reflects
  Qwen's generally higher per-token confidence and contributes to the calibration
  artifact observed in Experiments~1 and~3.}
\label{fig:qwen-signals}
\end{figure}

\section{Table: Liveness Fallback Frequency}
\label{app:liveness}

\begin{table}[H]
\centering
\caption{Liveness fallback frequency by condition, $N$, $\beta$, and model
  (averaged over fault types). At $\beta=0.45$ with $N=7$,
  $\lfloor 7 \times 0.45 \rfloor = 3$ faults; the BFT threshold is $2f+1=5$
  agents. With 3 F1 or F3 agents filtered by Module~1, only 4 agents remain,
  which is below threshold, and the liveness fallback triggers at 100\% of
  questions.}
\label{tab:liveness}
\small
...
\midrule
LLaMA 3.1 8B & \texttt{hard\_only}   & 7 & 5.0\%   & 7.5\%   & 18.8\% & 100\% (F1/F3) \\
LLaMA 3.1 8B & \texttt{full\_system} & 7 & 5.0\%   & 7.5\%   & 18.8\% & 100\% (F1/F3) \\
Qwen2.5 7B   & \texttt{hard\_only}   & 7 & 13.75\% & 16.25\% & 26.25\% & 100\% (F1/F3) \\
Qwen2.5 7B   & \texttt{full\_system} & 7 & 13.75\% & 16.25\% & 26.25\% & 100\% (F1/F3) \\
\bottomrule
\end{tabular}
\end{table}

\texttt{hard\_only} and \texttt{full\_system} are therefore equivalent to the
unfiltered baseline under F1/F3 at $\beta=0.45$, $N=7$. The difference between
conditions at $\beta=0.45$ in Tables~\ref{tab:exp1-llama}
and~\ref{tab:exp1-qwen} is driven by F2 (Byzantine) questions, where spoofed
logprobs pass Module~1 and filtering remains active.
...
The Experiment~1 runner is \texttt{eval/runner\_v2.py}; the Experiment~2
signal-quality runner is \texttt{eval/signal\_quality.py}; the Experiment~3
coordinated-injection runner is \texttt{eval/adversarial\_test\_v2.py}.
...
cache loading and answer extraction helpers are in \texttt{eval/io.py} and
\texttt{eval/answer\_extraction.py}.
...
files under \texttt{results/exp3\_*}, and the multi-provider CSVs reported in
Section~\ref{sec:multi}. No confidence intervals are reported because the
canonical Experiment~1 and Experiment~3 CSVs contain aggregate accuracies rather
than per-question correctness vectors.
