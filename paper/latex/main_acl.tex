...
model, strict answer extraction corrects a 16.5 percentage point
accuracy collapse in LLaMA majority-vote results at high fault fractions, and
the all-fault aggregate shows the strongest $\beta=45\%$ result for TopKMass
filtering plus strict majority vote. TopKMass is the strongest of three tested
...
Experiment~1 F2 logprobs are spoofed to TopKMass$=1.00$ (not 0.9999) because
...
The primary Experiment~1 result is methodological: strict answer extraction
corrects a 16.5 percentage point accuracy collapse in LLaMA
majority-vote results at high fault fractions, and the all-fault aggregate shows
the strongest $\beta=45\%$ result for TopKMass filtering plus strict majority
vote. Experiment~2 validates TopKMass as the strongest of the tested
...
conditions at $\beta=0.45$ in Table~\ref{tab:exp1-results} is driven by
Experiment~1 F2 questions, where spoofed logprobs pass Module~1 and filtering
remains active.
