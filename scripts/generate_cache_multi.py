#!/usr/bin/env python3
"""Phase 1: Offline vLLM inference → provider-tagged cache JSON.

Identical to scripts/generate_cache.py but adds --provider-name so that
each generation record carries model_id and provider fields required by
pipeline_multi.
...
def _build_records(
    ...
    for q, req_out in zip(questions, results):
        generations = [
            {
                "agent_id": f"q{q['question_id']}_{provider}_a{agent_idx}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a provider-tagged cache JSON via vLLM (Phase 1)."
    )
    parser.add_argument(
        "--model", required=True,
    ...
    parser.add_argument(
        ...
        help="Short provider tag, e.g. 'mistral' or 'phi3'.",
    )
    parser.add_argument(
        "--n-questions", type=int, default=50,
        help="Number of questions from each dataset (default: 50).",
    )
    parser.add_argument(
        "--output", default="cache.json",
        help="Destination path for the generated cache file.",
    )
    ...
    print(f"Generating {len(gsm_prompts)} GSM8K × 7 completions...", flush=True)
    gsm_results = llm.generate(gsm_prompts, gsm_params) if gsm_prompts else []

    print(f"Generating {len(sqa_prompts)} StrategyQA × 7 completions...", flush=True)
    sqa_results = llm.generate(sqa_prompts, sqa_params) if sqa_prompts else []

    all_records = (
        _build_records(gsm_questions, gsm_results, args.model, args.provider_name)
        + _build_records(sqa_questions, sqa_results, args.model, args.provider_name)
    )
