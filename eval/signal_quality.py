            ...
            for ft in fault_types:
                ...
                for gen in injected:
                    ...
                    if signals is None:
                        continue
                    is_correct = (
                        _extract_answer(gen.output_text, ground_truth)
                        == ground_truth.strip()
                    )
                    rows.append(
                        {
...
def plot_signals(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.scatter(
        df.loc[incorrect_mask, "topk_mass"],
        y_jittered[incorrect_mask],
        alpha=0.2,
        s=8,
        color="#d62728",
        label="Incorrect",
    ...
    ax.scatter(
        df.loc[correct_mask, "topk_mass"],
        y_jittered[correct_mask],
        alpha=0.2,
        s=8,
        color="#2ca02c",
        label="Correct",
    ...
    for is_correct, color in [(False, "#d62728"), (True, "#2ca02c")]:
        ...
        ax.axvline(
            med,
            ymin=(y_pos - 0.1 + 0.5) / 1.14,
            ymax=(y_pos + 0.1 + 0.5) / 1.14,
            color=color,
            linewidth=2.5,
            alpha=0.9,
        )
    ...
    if has_faults:
        ...
        ax.plot(
            [0, 1], [0, 1], color="gray", linestyle="--", linewidth=1, label="Chance"
        )
        ax.plot(
            fpr,
            tpr,
            color=_COLORS["topk_mass"],
            linewidth=2,
            label=f"TopKMass (AUC={auc:.3f})",
        )
        ax.set_title(
            "Fault Detection ROC\n(−TopKMass as detector)",
            fontsize=12,
            fontweight="bold",
        )
        ax.set_xlabel("False Positive Rate", fontsize=11)
...
def run_experiment_2(
    ...
    df = analyze_cache(
        cache_path, include_faults=include_faults, beta=beta, fault_types=fault_types
    )
    n_clean = int((df["fault_type"] == "clean").sum())
    n_faulty = int(df["is_faulty"].sum()) if include_faults else 0
    print(
        f"  {n_clean} clean generations | {df['is_correct'].sum()} correct "
