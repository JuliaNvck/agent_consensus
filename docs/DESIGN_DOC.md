...
- **Implementation notes:**
  - **Cache format:** JSON file with `{"questions": [{"question_id", "ground_truth", "generations": [{agent fields}]}]}`. Minimum 7 agents per question (to support N=7). `eval.io.load_cache(filepath)` returns `List[Tuple[str, List[AgentGeneration]]]` for homogeneous runners.
  - **Answer extraction (`eval/answer_extraction.py`):** `extract_answer(output_text, ground_truth)` normalizes chain-of-thought outputs before scoring. `extract_answer_strict(output_text, ground_truth)` returns `None` when no parseable yes/no or numeric answer is present. `answer_majority_voting` and `answer_majority_voting_strict` implement the corresponding answer-level votes.
  - **Baseline functions (`eval/baselines.py`):** `majority_voting(agents)` uses `collections.Counter.most_common(1)` on raw `output_text`. Answer-voting helpers are re-exported from `eval.answer_extraction` for compatibility. `soft_weighted_geometric_median` computes per-agent TopKMass mean as weight, runs a weighted Weiszfeld L-BFGS-B minimisation (`_weighted_geometric_median`), returns text of nearest agent. Falls back to uniform weights when all weights < 1e-15 (all F1 agents).
  ...
  - **Stage 1 wrapper:** `pipeline.stage1.nearest_centroid_text(admitted)` is the shared geometric median nearest-centroid selector used by final `stage1_only` paths. It reuses `pipeline.aggregation._embed` and `_geometric_median` without changing the math.
  - **Liveness fallback in runner:** `_run_condition` replicates orchestrator logic directly (`await filter_agents(agents, tau)` + `if len(admitted) < 2f+1: fallback`) without the asyncio.sleep broadcast overhead. `f = (N-1)//3` (BFT standard: N=5→f=1, threshold=3; N=7→f=2, threshold=5). In final V4 homogeneous runs, `full_system` uses the stage1-only nearest-centroid selector rather than NLI.
...
Figures regenerated (2026-05-11) from V4 CSVs via `scripts/plot_results.py`.

Four files in `figures/`:
- `accuracy_vs_beta.png` — two-panel line chart (LLaMA left, Qwen right) with N=1 reference line; supplementary/appendix
- `accuracy_vs_beta_no_n1.png` — same chart without N=1 line; main paper body
- `fault_type_breakdown.png` — grouped bar chart, all 4 conditions × 4 fault types at β=45%
- `adversarial_coordination.png` — two-panel line chart for Exp 3: accuracy vs. coordination degree
...
**stage1_only** *(recommended)*: Module 1 filter + liveness fallback → geometric median nearest-neighbor (no NLI Stage 2). Implemented through `pipeline.stage1.nearest_centroid_text`, which reuses `pipeline.aggregation._embed` and `_geometric_median`. This is the condition that beats majority vote under coordinated attack (LLaMA: 70% vs 62.5%).

**full_pipeline** *(included for comparison only; consistently worst)*: Module 1 filter + `pipeline_v2.aggregation.aggregate()`, the legacy distance-weighted vote ablation. This is not the NLI path in reported `_v2.csv` Exp 3 results. It underperforms `stage1_only` in the coordinated valid-format attack setting and should be discussed as a failed weighted-vote ablation.
...
**2. `full_pipeline` (weighted-vote ablation) is the worst condition in every scenario.** LLaMA: 57.5%–56.25% across all coordination degrees, consistently 10–14pp below stage1_only. Qwen: 48.75% across all conditions, below both majority vote and stage1_only. The reported `_v2.csv` rows use `pipeline_v2.aggregation.aggregate()`, not NLI Stage 2. **`full_pipeline` should not be used as the final method; `stage1_only` is the correct geometric-median condition.**
...
## 10. Multi-Provider Heterogeneous Setting (Experiments A & B)

This section covers both the conceptual extension to multi-provider pools and the measured results from Experiments A and B, which tested this hypothesis using four open-weight models as independent providers. Results are in `results/exp_multi_a_diversity.csv`, `results/exp_multi_b_phi3.csv`, and `results/exp_multi_b_mistral.csv`.
...
### 10.4 Experiment A: Provider Diversity (`results/exp_multi_a_diversity.csv`)

**Research question:** Does a 4-provider mixed pool (LLaMA, Qwen, Mistral, Phi-3) outperform the best individual provider?

**Setup:** Mixed cache with 2-2-2-1 agents per question from four providers; no fault injection. Geometric median nearest-centroid (`pipeline_multi/aggregation.py`) with per-provider τ calibration.
...
**Findings:** Multi-provider consensus does not beat the best single provider. It lands 1.25pp below single_llama (67.5% vs 68.75%) and 1.25pp above single_phi3/single_qwen. Mistral is unexpectedly weak (56.25%), contributing noise rather than signal. The multi-provider geometric median partially absorbs Mistral's drag — it beats single_mistral by 11.25pp — but not enough to overcome the dilution of LLaMA's stronger individual accuracy.

This is the expected neutral/slightly-negative outcome the plan anticipated: when one provider is weak and there is no coordination among wrong answers, the mixed pool's geometric median gravitates toward the agreement of the three stronger providers but cannot exceed the accuracy of the strongest one alone.

### 10.5 Experiment B: Biased Provider (`results/exp_multi_b_phi3.csv`, `results/exp_multi_b_mistral.csv`)

**Research question:** Can geometric median absorb a systematically biased provider — one whose answers are consistently wrong?

**Setup:** Two designated biased providers (phi3, mistral) × two bias types (natural = as-is; injected = forced wrong answers via F1/F2 injection). Three conditions per combination:
- `no_[provider]` — oracle baseline: remove the biased provider entirely
- `majority_vote_all` — naive plurality over all 7 agents
- `geometric_median_all` — our system

**Results — phi3 as biased provider:**

| Condition | Bias type | Accuracy | Fallback freq |
|---|---|---|---|
| no_phi3 (oracle) | natural | 0.675 | 0.050 |
| majority_vote_all | natural | **0.700** | 0.000 |
| geometric_median_all | natural | 0.675 | 0.038 |
| no_phi3 (oracle) | injected | 0.675 | 0.050 |
| majority_vote_all | injected | **0.700** | 0.000 |
| geometric_median_all | injected | 0.675 | 0.050 |

**Results — mistral as biased provider:**

| Condition | Bias type | Accuracy | Fallback freq |
|---|---|---|---|
| no_mistral (oracle) | natural | 0.700 | 0.075 |
| majority_vote_all | natural | **0.700** | 0.000 |
| geometric_median_all | natural | 0.675 | 0.038 |
| no_mistral (oracle) | injected | 0.700 | 0.075 |
| majority_vote_all | injected | **0.7125** | 0.000 |
| geometric_median_all | injected | 0.650 | 0.075 |

**Findings:** Majority vote beats geometric median in every condition. When Mistral is injected (forced wrong answers), geometric median degrades further (0.65 vs majority vote's 0.7125) while majority vote slightly improves — a strong negative result. Removing the biased provider (oracle) achieves only 0.675–0.700, showing the biased provider's wrong answers are minimally harmful anyway.

### 10.6 Interpretation: Scope of the Defense

These results sharpen — rather than undermine — the paper's central claim. The contrast between Exp 3 and Exp B is exact:

**Geometric median wins when wrong agents are coordinated on the same answer (Exp 3):** 2 Byzantine agents agree on `"The answer is no."` → tight cluster in embedding space → majority vote swings wrong. Geometric median resists the cluster and recovers +7.5pp (LLaMA).

**Geometric median does not win when wrong agents produce diverse failures (Exp B):** A biased provider generates different wrong answers per question — no coordination, no tight cluster. 1–2 diverse wrong votes out of 7 are trivially outvoted by majority. Geometric median has no edge because there is no adversarial cluster to resist.

The mechanism is cluster cohesion, not minority size. The centroid shift metric (§9.5) confirms this: geometric median's robustness advantage comes from staying outside a tight wrong-answer cluster. When wrong answers are scattered, both methods converge on the same honest majority and the geometric median's additional overhead provides no benefit.

**Paper framing — "scope of the defense":**
> Geometric median over embedding space is not universally superior to majority vote. Its advantage requires a specific threat: a coordinating minority that concentrates votes on the same wrong answer. Against uncoordinated failures — random fault injection (Exp 1 F1/F3), natural provider heterogeneity (Exp A), or diverse provider bias (Exp B) — majority vote is simpler and at least as effective. The pipeline's layered design (Module 1 for invalid-format failures, Module 2 for coordinated semantic attacks) is correct: each layer addresses its specific failure mode and neither overclaims universality.

This is a defensible and honest result. Knowing *when* a technique helps and *why* — with a mechanistic explanation via centroid shift — is stronger than an unqualified accuracy win.
